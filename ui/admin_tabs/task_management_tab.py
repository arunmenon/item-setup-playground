# File: ui/admin_tabs/task_management_tab.py
...
def create_task_management_tab(admin_db_handler):
    with gr.TabItem("Task Management"):
        ...
        def get_task_options(task_type):
            tasks = (
                admin_db_handler.get_generation_tasks()
                if task_type=='Generation'
                else admin_db_handler.get_evaluation_tasks()
            )
        ...
        # Columns: name, type, range, required, categories
        # e.g. "name=decision, type=yes_no" -> we'll parse Yes/No as 1/0
        metrics_table = gr.Dataframe(
            headers=["name", "type", "range", "required", "categories"],
            row_count=3,
            col_count=5,
            type="array",
            visible=False,  # shown only if 'Evaluation'
            interactive=True
        )
        ...
        def update_form(task_type):
            ...
            return (
                ...
                "",
                gr.update(visible=show_metrics)  # metrics_table
            )
        ...
        task_type_selector.change(
            ...
            outputs=[
                selected_task, task_name, description,
                max_tokens, output_format, delete_button,
                feedback, metrics_table
            ]
        )
        ...
        def load_task(task_name_val, task_type_val):
            ...
            if task_name_val in tasks:
                ...
                task = (
                    admin_db_handler.db_session
                    .query(task_model)
                    .filter_by(task_name=task_name_val)
                    .first()
                )
                if task:
                    metrics_data = []
                    if task_type_val == 'Evaluation' and hasattr(task, 'expected_metrics') and task.expected_metrics:
                        em = json.loads(task.expected_metrics)  # e.g. {"metrics":[{"name":"score","type":"float"},...]}
            ...
            return (
                ...
                gr.update(visible=(task_type_val=='Evaluation'), value=[])
            )
        ...
        def convert_table_to_json(data):
            ...
            for row in data:
                ...
                if metric["type"] == "categorical":
                    cats_str = row[4].strip() if row[4] else ""
                    cats_list = [c.strip() for c in cats_str.split(",") if c.strip()]
                    metric["categories"] = cats_list
        ...
        save_button.click(
            fn=save_task,
            inputs=[task_id, task_name, description, max_tokens,
                    output_format, task_type_selector, metrics_table],
        ...
        def delete_task(task_id_val, task_type_val):
            ...
            try:
                ...
                if task:
                    ...
                    admin_db_handler.db_session.commit()
                    return "Task deleted successfully.", gr.update(
                        choices=get_task_options(task_type_val), value=None
                    )
                else:
                    return "Task not found.", gr.update()
            except Exception as e:
