...
def create_task_management_tab(admin_db_handler):
    with gr.TabItem("Task Management"):
        ...
        def update_form(task_type):
            ...
            return gr.update(choices=options, value=None), "", "", gr.update(visible=False), ""

        task_type_selector.change(
            fn=update_form,
            inputs=[task_type_selector],
            outputs=[selected_task, task_name, description, delete_button, feedback]
        )
        ...
        def load_task(task_name_val, task_type_val):
            ...
            if task_name_val in tasks:
                ...
                if task:
                    return str(task.task_id), task.task_name, task.description, gr.update(visible=True), ""
            return "", task_name_val, "", gr.update(visible=False), "Ready to create a new task."

        selected_task.change(
            fn=load_task,
            inputs=[selected_task, task_type_selector],
            outputs=[task_id, task_name, description, delete_button, feedback]
        )

        # Save task
        def save_task(task_id_val, task_name_val, description_val, task_type_val):
