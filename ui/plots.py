...
def generate_correlation_heatmap(evaluation_df):
    ...
    evaluation_df['evaluation_data_parsed'] = evaluation_df[
        'evaluation_data'].apply(lambda x: json.loads(x) if pd.notnull(x) else {})
...
def generate_winner_model_comparison_plot(evaluation_df):
    ...
    plot_df['Evaluator'] = plot_df['Evaluator'].apply(lambda x: 'Human' if x.endswith('_human') else (
        'LLM' if x.endswith('_llm') else x))
